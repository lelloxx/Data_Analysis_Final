...
# Preparing the data
# Since 'Outlet_Type' is one-hot encoded, we include the dummy variables directly
x_train <- model.matrix(Item_Outlet_Sales ~ Item_MRP + Outlet_Establishment_Year + Outlet_Size +
                           `Outlet_Type.Grocery Store` + `Outlet_Type.Supermarket Type1` +
                           `Outlet_Type.Supermarket Type2` + `Outlet_Type.Supermarket Type3` + 
                           I(Item_MRP * Outlet_Size) - 1, data = train_data)

y_train <- train_data$Item_Outlet_Sales

x_test <- model.matrix(Item_Outlet_Sales ~ Item_MRP + Outlet_Establishment_Year + Outlet_Size +
                          `Outlet_Type.Grocery Store` + `Outlet_Type.Supermarket Type1` +
                          `Outlet_Type.Supermarket Type2` + `Outlet_Type.Supermarket Type3` + 
                          I(Item_MRP * Outlet_Size) - 1, data = test_data)
...
# Preparing the data
# Using only the variables selected by stepwise regression
x_train <- model.matrix(Item_Outlet_Sales ~ Item_MRP + Outlet_Establishment_Year + Outlet_Size +
                           `Outlet_Type.Grocery Store` + `Outlet_Type.Supermarket Type1` +
                           `Outlet_Type.Supermarket Type2` + `Outlet_Type.Supermarket Type3` + 
                           I(Item_MRP * Outlet_Size) - 1, data = train_data)
...
selected_vars <- c("Item_MRP", "Outlet_Establishment_Year", "Outlet_Size",
                   "Outlet_Type.Grocery Store", "Outlet_Type.Supermarket Type1",
                   "Outlet_Type.Supermarket Type2", "Outlet_Type.Supermarket Type3",
                   "MRP_x_Outlet_Size", "Item_Outlet_Sales")

# Check to ensure all selected variables are in train_data and test_data
if (!all(selected_vars %in% names(train_data))) {
    stop("One or more selected variables not found in train_data")
}
if (!all(selected_vars %in% names(test_data))) {
    stop("One or more selected variables not found in test_data")
}
...
dtrain <- xgb.DMatrix(data = as.matrix(train_data_selected[, -which(names(train_data_selected) == "Item_Outlet_Sales")]), 
                      label = train_data_selected$Item_Outlet_Sales)
dtest <- xgb.DMatrix(data = as.matrix(test_data_selected[, -which(names(test_data_selected) == "Item_Outlet_Sales")]), 
                     label = test_data_selected$Item_Outlet_Sales)
