...
Group members:
  ...
  - Alessandro Maria Austeri 307951
  - Gianmarco Mancini        285151
...
```{r dropping useless, include=FALSE }
df <- df %>%
...
```{r categories reduction of "Item_Type", echo=FALSE}
df <- df %>%
...
```{R Relationship between Item MRP and Outlet Sales by Product Type, echo=FALSE}
df_summary <- df %>%
...
```
Before proceeding with the application of the models we transformed the `Outlet_Establishment_Year` into `Outlet_Age` to be more comfortable in our analysis later.
...
```
We also performed ensembling, and as observed, the performance remains similar, with the R-squared value still around 0.57. Given that the performance did not improve significantly even with stacking, we will proceed to add new features to our model through another feature engineering to identify additional factors that could potentially enhance the predictive power and provide a better understanding of the variables influencing sales.
...
```
#### Stepwise Regression with new features (price per unit weight, log of item_visibility, mrp * outlet_size )
```{R stepwise regression 2, include=FALSE}
...
#cat("R-squared:", train_r_squared, "\n")
```
We can see that the rmse and $R^2$ values are the same and the list of features chosen by the model it's also the same, so, the new features have not improved the prediction level, because of this, we will not run again every model with the new features.



```{r random forest tree using only selected variables, include=FALSE, warning=FALSE}
...
```
With the random forest we can see a slight increase in the $R^2$ value, (0.03), after this we will tune the parameter of the rft (mtry and min node size).


```{r random forest tree mtry and node size number detection, include=FALSE, warning=FALSE}
...
To find the optimal number of clusters we used the Elbow Method and the last point before the curve start increasing is 9 so the perfect number should be 9.
...
```{r cluster analyzing plots, echo=FALSE}
if (!require("ggplot2")) install.packages("ggplot2", dependencies = TRUE)
...
```{R cluster on item fat content, , echo=FALSE}
...
Our last analysis is the distribution of item MRP by item fat content, in order to see if product with low fat are more likely to be in the "High price" cluster, but, the evidence from the plots is that the distribution amongside the two clusters is the same for both values of item fat content.
We think that this is given by the fact that the dataset is from 2013 and our world start to go to a more fit dimension just after covid.