...
```
abbiamo visto che se inizia con L appartiene a Low fat, altrimenti a Regular Fat.
...
```{r Outlet size analysis, echo=FALSE}
...
```{r Outlet_Size Analysis 2, echo=FALSE}
#conteggio records senza Outlet_Size:
null<-sum(nchar(df$Outlet_Size) ==0)
...
##checking for outliers before removing 0
...
since there are few outliers in the item visibility, we have decided to remove them.

#scrivere il codice per rimuovere gli outliers
...
#category reduction of Item_Type
...
```
we have converted item_type_category into:
Food basics 
Processed Foods
Beverages
Non-Food Items
...
Per avere risultati più basati sul tipo di prodotto e non sul prodotto in se
...
Outlet_Size and Item_Outlet_Sales, it seems that the bigger the outlet, the higher the sales

Item_MRP and Item_Outlet_Sales, an higher mrp lead to an increase in sales of that product

Small negative correlation betwen item_visibility and outlet_size 

Small negative correlation between item_visibility and item_outlet_sales

#Item MRP and Outlet Sales

as stated in the corr matrix wecan see that the higher the MRP, the higher the outlet sales.
In the following plot we can focus on the "blank zones" in which there are not sales.
We have findout that there 

```{r item MRP vs Outlet sales with gaps of price, echo=FALSE }
...
```{r Advanced visualization Visibility-Sales, echo=FALSE, message=FALSE}
# Load necessary libraries
library(dplyr)
library(ggplot2)
...
scriviamo a mano cosa abbiamo fatto e l'output



```{r Distribution of Variables across Item Sales, echo=false}
# Load necessary libraries
library(ggplot2)
library(dplyr)
...
#metti in grid i grafici ti average sales by oytlet_sizes, scatterplot di item visibility vs outlet sales ed i due di prima.
...
```
scriviamo che gli item con la visibilità più alta sono quelli con l ' mrp più basso

gli item che fanno vedere di può sono quelli che vendono a di meno quindi quelli di cui vogliono sbarazzarsi.

```{r Segmented Analysis Regression, include=FALSE}
# Load necessary libraries
library(ggplot2)
library(dplyr)
...
```
abbiamo controllato che non ci sono outliers ma solo qualche valore alto in item sales e item visibility che però potrebbero essere veri
...
```{r Impact of item MRP ad Outlet Type on Sales, include=FALSE }
...
from establishmenbt year to age 
...
```
abbiamo fatto la one hot su outlet type e item_Type
...
Il valore R-squared per tutti i metodi non linear che abbiamo usato è simile, iintorno a 0.57 sia con tutte le variabili che con solo le variabili selezionate dalla stepwise regression, ora proviamo diverse tecniche per aumentare l r-square del nostro modello:
...
come possiamo vedere anche con lo stacking la performance è simile
...
vediamo se la stepwise regression ora include anche variabili che abbiamo aggiunto con la feature
...
qui dobbiamo stampare come sempre solo r square e rmse.
...
```{r random forest tree using only selected variables, echo=FALSE}
